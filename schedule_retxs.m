...
%ordenamos de menor a mayor según el momento de
% retx de cada copia enviada por una tx unicast diferente para un UE
for i=1:length(x)
...
while 1
    ...
    if flag==0
        if strcmp(scheduling,'GF')
            ...
            if flag==0
                disp('flag es igual a cero. No debería estar aquí\n');
            end
        end
    end
    if flag==2
    ...
    if flag==1
        if fretx==1
            ...
            if retx(user,ind_user,1)<=maxN_retx
                ...
                ACK_time=auxtime+(Tslot-auxtime2)+Tslot/14; %(Tslot-auxtime2) es el frame alignment hasta el siguiente slot
